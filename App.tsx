...
const i18n = {
  zh: {
    ...
    saveProfile: "保存",
    cancel: "取消",
    disclaimer: "内容由 AI 生成，不代表平台立场",
    reportTitle: "举报内容",
    reportPlaceholder: "请描述违规内容...",
    reportSubmit: "提交举报",
    reportSuccess: "举报已提交，平台将在24小时内处理",
    report: "意见反馈",
    menu: "菜单",
    newTopicCost: "新话题",
    answerCost: "每次回答",
  ...
  en: {
    ...
    saveProfile: "Save",
    cancel: "Cancel",
    disclaimer: "Content generated by AI, does not represent the platform's views",
    reportTitle: "Report Content",
    reportPlaceholder: "Please describe the violation...",
    reportSubmit: "Submit Report",
    reportSuccess: "Report submitted. We will process within 24 hours.",
    report: "Report",
...
const App: React.FC = () => {
  ...
  const [reportText, setReportText] = useState(''); // 举报内容
  const [reportSubmitted, setReportSubmitted] = useState(false); // 举报是否已提交
...
5. Keep within 150-200 words, be incisive and comprehensive`;
            ...
            <div className="bg-white w-full max-w-md rounded-[2.5rem] shadow-2xl p-8 space-y-8 relative">
              ...
              <div className="space-y-6">
                ...
                <div className="pt-2 space-y-3">
                    {/* 意见反馈按钮 */}
                    <button onClick={() => setShowReport(true)} className="w-full py-3 bg-slate-100 text-slate-600 rounded-2xl font-bold hover:bg-slate-200 transition-all flex items-center justify-center gap-2">
                      <Mail size={16} />
                      {t.report}
                    </button>
  ...
  return (
    <div className="flex flex-col h-screen bg-slate-50">
      ...
      {showReport && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md">
          <div className="bg-white w-full max-w-sm rounded-[2rem] shadow-2xl p-6 space-y-5">
            <div className="flex items-center justify-between">
              <h3 className="text-lg font-bold text-slate-900">{t.reportTitle}</h3>
              <button onClick={() => { setShowReport(false); setReportText(''); setReportSubmitted(false); }} className="p-2 hover:bg-slate-100 rounded-full">
            ...
            {reportSubmitted ? (
              <div className="text-center py-6 space-y-3">
                ...
                <p className="text-slate-600">{t.reportSuccess}</p>
                <button onClick={() => { setShowReport(false); setReportText(''); setReportSubmitted(false); }} className="px-6 py-2 bg-slate-900 text-white rounded-xl font-medium hover:bg-black transition-all">
                  {t.cancel}
                </button>
              </div>
            ) : (
              <>
                <p className="text-sm text-slate-500">{lang === 'zh' ? '举报邮箱：contact@vinolab.tech' : 'Report email: contact@vinolab.tech'}</p>
                <textarea
                  value={reportText}
                  onChange={e => setReportText(e.target.value)}
                  placeholder={t.reportPlaceholder}
                  rows={4}
                  className="w-full px-4 py-3 bg-slate-50 rounded-xl border-2 border-slate-200 focus:border-blue-500 outline-none text-base resize-none"
                />
                <button
                  onClick={() => { if (reportText.trim()) setReportSubmitted(true); }}
                  disabled={!reportText.trim()}
                  className="w-full py-3 bg-red-500 text-white rounded-xl font-bold hover:bg-red-600 transition-all disabled:opacity-40 disabled:cursor-not-allowed"
                >
                  {t.reportSubmit}
                </button>
                <p className="text-xs text-center text-slate-400">{lang === 'zh' ? '平台承诺在24小时内完成核实处理' : 'We will process within 24 hours'}</p>
              </>
            )}
          </div>
        </div>
